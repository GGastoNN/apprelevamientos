...
class OpeningDetector {
    ...
    fun detect(source: Bitmap): List<Candidate> {
                ...
                for (top in 0 until h - minH step max(6, h / 28)) {
                    for (bottom in top + minH until h step max(8, h / 22)) {
                        ...
                        if (border > 0.58f) {
                            ...
                            val confidence =
                                (border * 0.82f + min(0.18f, area * 0.25f))
                                    .coerceAtMost(0.98f)
        ...
        val sorted: List<Candidate> =
            out.sortedByDescending { candidate: Candidate ->
                candidate.confidence
            }

        val filtered = mutableListOf<Candidate>()

        for (candidate: Candidate in sorted) {
            val overlapsExisting =
                filtered.any { existing: Candidate ->
    ...
    private fun borderScore(
        ...
        for (x in l..r step sx) {
            p(x, t)
            p(x, b)
        }

        for (y in t..b step sy) {
            p(l, y)
            p(r, y)
        }
